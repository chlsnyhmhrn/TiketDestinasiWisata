...
<body>
    ...
    <div class="flex flex-col h-screen md:flex-row bg-gray-100">
        ...
        <div class="w-full bg-white p-8 rounded-2xl shadow-md">
            <p class="text-3xl font-bold">Riwayat Pesanan</p>

            <!-- Search Form -->

            <div class="flex flex-col mt-5">
                <form id="searchForm" method="GET"
                    action="{{ route('penjual.riwayat_pesanan', $destinasi->id_destinasi) }}"
                    class="flex items-center gap-2 mb-2">
                    <input autocomplete="off" type="text" name="search" placeholder="Cari"
                                    ...
                                    @if ($tiket->count() > 0)
                                        @foreach ($tiket as $item)
                                            <tr>
                                                ...
                                                <td class="font-semibold">
                                                    @if ($item->status == 'Disetujui')
                                                        <a href="{{ route('penjual.invoice', $item->id_tiket) }}"
                                                            class="btn btn-sm text-sm text-white btn-warning">Invoice</a>
                                                    @else
                                                        <a href="{{ route('penjual.invoice', $item->id_tiket) }}"
                                                            aria-disabled="true"
                                                            class="btn btn-sm text-sm text-white btn-warning btn-disabled">Invoice</a>
                                                    @endif
                                                </td>
                                            </tr>
                                        @endforeach
